CREATE DEFINER=`root`@`localhost` PROCEDURE `GenerateSampleContributionList`(
	_StateCode CHAR(2),
    _City VARCHAR(100),
    _Year INT,
    _NumRows INT
...
DECLARE _CheckNum int;
DECLARE _ContrDate date;
DECLARE _MonthCtr int;
DECLARE _ContrAmount decimal(8,2);
    
SET _Diff = 0.2;
SET _Counter = 1;

DROP TABLE IF EXISTS TempNamesList;
CREATE TEMPORARY TABLE TempNamesList (
    ...
    ContributionAmount decimal(8,2) not null);
...
WHILE (_Counter <= _NumRows) DO
	...
	#Now set contribution amounts
	SET _MonthCtr = 1;
	SET _CheckNum = GenerateRandomNumber(4);
	SET _ContrAmount = 20 + (ROUND((RAND() * 120), 0) * 5);
	WHILE (_MonthCtr <= 12) DO
		SET _ContrDate = GenerateSampleDate(_year, _MonthCtr, 1);
		SET _CheckNum = _CheckNum + 5;
		IF ((_MonthCtr mod 2) = 1) THEN
			SET _ContrAmount = _ContrAmount + 20;
		END IF;
	
		INSERT INTO TempNamesList(ContributorID, FirstName, LastName, MI, Gender, EMail, Address, City, 
			StateCode, ZipCode, PhoneNumber, BirthDate, CheckNumber, ContributionDate, ContributionAmount)
		SELECT
			_Counter AS ContributorID,
			...
			_CheckNum AS CheckNumber,
			_ContrDate AS ContributionDate,
			_ContrAmount AS ContributionAmount;
	
		SET _MonthCtr = _MonthCtr + 1;
	END WHILE;

	SET _Counter = _Counter + 1;
END WHILE;

SELECT * FROM TempNamesList;
